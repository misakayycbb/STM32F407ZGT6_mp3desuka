...
uint16_t ILI9341_Dx_Pin[16] = {ILI9341_D0_PIN, ILI9341_D1_PIN, ILI9341_D2_PIN, ILI9341_D3_PIN, 
...
void ILI9341_Write_Cmd ( uint16_t usCmd )
{
	...
	for(i = 0; i < 16; i++)
	{
		if(((usCmd >> i) & 0x0001) == 0x0001)
		{
			if(i <= 3 || i >= 13)
				GPIO_SetBits(GPIOD, ILI9341_Dx_Pin[i]);
			else
				GPIO_SetBits(GPIOE, ILI9341_Dx_Pin[i]);
		}
		else
		{
			if(i <= 3 || i >= 13)
				GPIO_ResetBits(GPIOD, ILI9341_Dx_Pin[i]);
			else
				GPIO_ResetBits(GPIOE, ILI9341_Dx_Pin[i]);
		}
	}
	ILI9341_Delay(1);
...
uint16_t ILI9341_Read_Data ( void )
{
	...
	for(i = 0; i < 16; i++)
	{
		...
		if(i <= 3 || i >= 13)
		{
			if(GPIO_ReadInputDataBit(GPIOD, ILI9341_Dx_Pin[i]))
			{
				RetData |= 0x8000;
			}
		}
		...
		{
			if(GPIO_ReadInputDataBit(GPIOE, ILI9341_Dx_Pin[i]))
			{
				RetData |= 0x8000;
			}
		}
